...
bins_a = linspace(-pi, pi)
bins_lat = linspace(0,360,200)
mid_point_x, mid_point_y = meshgrid((bins_a[:-1]+bins_a[1:])/2, (bins_u[:-1]+bins_u[1:])/2 )
N_total = 0
N_total2 = 0
histo = zeros([31, 144, len(bins_u)-1,len(bins_a)-1])
histo_c = zeros([31, len(bins_u)-1,len(bins_a)-1])
...
#        indices = argwhere(datos['levels']<60)
        ...
        vwind = arctan2(datos['U'][:,3,:,:],datosv['U'][:,3,:,:])
        uwind = uwind.transpose([1,2,0])
        vwind = vwind.transpose([1,2,0])
#        hist_total += histogram2d(uwind[:],bins_u, bins_lat)[0] 
        for i in arange(31):
            for j in arange(144):
                histo_c[i,:,:] += histogram2d(uwind[i,j,:],vwind[i,j,:],[bins_u, bins_a])[0]
                histo[i,j,:,:]+= histogram2d(uwind[i,j,:],vwind[i,j,:],[bins_u, bins_a])[0]
...
lat = datos['lat']
lon = datos['lon']
Int = simps(histo_c,(bins_a[:-1]+bins_a[1:])/2, axis=-1)
Int = simps(Int,(bins_u[:-1]+bins_u[1:])/2, axis=-1)

savez('joint_pdf_ang', histo_c = histo_c/Int.reshape([-1,1,1]), histo = histo, lat = lat,lon = lon, bins_u = bins_u, bins_a = bins_a)        
Nlat = 15
figure()
...
lat = datos['lat']
lon = datos['lon']
plt.savefig('../../StratosphericPropulsion/PaperSerio/eps_foto/alphav_pdf.eps', dpi=1000)
...
ylabel(r'$\alpha$(rad)')
title(r'Función de densidad de probabilidad conjunta. Latitud: {} grados'.format(lat[Nlat]))
plt.savefig('../../StratosphericPropulsion/PaperSerio/eps_foto/alphav2_pdf.eps', dpi=1000)
